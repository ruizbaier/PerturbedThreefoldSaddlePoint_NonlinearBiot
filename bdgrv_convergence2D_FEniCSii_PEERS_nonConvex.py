...
fileO = XDMFFile("outputs/out-2D-convergence-nonConvex-bulk.xdmf")
fileO.parameters["functions_share_mesh"] = True
fileO.parameters['rewrite_function_mesh'] = True
fileO.parameters["flush_output"] = True

fileOM = XDMFFile("outputs/out-2D-convergence-nonConvex-phi.xdmf")
...
p_str = '(y-2*x)**2*y*cos(pi*x*y)'#had to construct a p exact that vanishes on Gamma... not sure why?
kappa_str = 'exp(x*y)'
...
for nk in range(nkmax):
    ...
    mesh = Mesh("meshes/nonConvex2DFiner%01g.xml"%nk)
    mesht = Mesh("meshes/nonConvex2D%01g.xml"%nk)

    facet_mesh = MeshFunction("size_t", mesh, "meshes/nonConvex2DFiner%01g_facet_region.xml"%nk)
    facet_mesht = MeshFunction("size_t", mesht, "meshes/nonConvex2D%01g_facet_region.xml"%nk)
    ...
    n = FacetNormal(mesh)
    n_ = OuterNormal(bmesht, [0.25, 0.15])  # a point inside the domain
